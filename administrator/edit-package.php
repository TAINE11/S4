...
$server = $connect->query("SELECT * FROM Server WHERE uname = '".$_GET['server']."'")->fetch_array();
...
$query = $connect->query("SELECT * FROM HostPackage WHERE id = '".$_GET['id']."'")->fetch_array();

if(empty($_GET['id']) || $_GET['id'] != $query['id']){
    echo swal('ID Gói Không Hợp Lệ','error');
    echo redirect('./server.php');
}

if(isset($_GET['delete_server'])){
    $check = $connect->query("SELECT * FROM `HostPackage` WHERE `id` = '".$_GET['delete_server']."'")->fetch_array();
    
    if($_GET['delete_server'] != $check['id']){
        echo swal('ID Máy Chủ Không Hợp Lệ','error');
    } else {
        $inTrue = $connect->query("DELETE FROM `HostPackage` WHERE `id` = '".$_GET['delete_server']."'");
        if($inTrue){
...
<div class="content-page">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box">
                                <h4 class="page-title"> Chỉnh Sửa Gói </h4>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-lg-6">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="header-title"> Chỉnh Sửa Thông Tin Gói </h4>
                                ...
                                <div class="card-body">
                                    <div class="needs-validation" novalidate="">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="packages" placeholder="VD: package1" required="" value="<?=$query['package'];?>">
                                        </div>
                                        
                                        <div class="mb-3">
                                            <label class="form-label" for="name"> Dung Lượng </label>
                                            <input type="text" class="form-control" id="disk" placeholder="VD: 500 MB" required="" value="<?=$query['disk'];?>">
                                        </div>
                                        
                                        
                                        <div class="mb-3">
                                            <label class="form-label" for="uname"> Băng Thông </label>
                                            <input type="text" class="form-control" id="bangthong" placeholder="VD: Không Giới Hạn" required="" value="<?=$query['bandwidth'];?>">
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                <input type="text" class="form-control" id="addondomain" placeholder="VD: Không Giới Hạn" required="" value="<?=$query['addondomain'];?>">
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                <input type="text" class="form-control" id="subdomain" placeholder="VD: Không Giới Hạn" required="" value="<?=$query['subdomain'];?>">
                                            </div>
                                        </div>
                                        ...
                                        <button class="btn btn-primary" onclick="ThemMayChu()"> Cập Nhật Gói </button>
                                    </div>
                            ...
                            <div class="card">
                                ...
                                <div class="table-responsive-sm">
                                        <table class="table mb-0">
                                            ...
                                            <tbody>
                                                <?php
                                                $query = $connect->query("SELECT * FROM HostPackage WHERE server = '".$server['uname']."'");
        ...
        <script>
            function ThemMayChu(){
                  $.ajax({
                    url: "/administrator/ajaxs/server.php",
                    method: "POST",
                    data: {
                        type: 'editgoi',
