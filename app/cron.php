...
$connect->query("UPDATE System SET date_cron = '".time()."'");
...
$num_Momo = $getMomo->num_rows;
    ...
    $explode = explode("csv ", $getMomo['comment']);
    
    $queryUser = $connect->query("SELECT * FROM Users WHERE username = '".$explode[1]."'")->fetch_array();
    
    if($queryUser['username'] == $username){
    $inTrueM = $connect->query("UPDATE Users SET money = money + '".$fetch_Momo['price']."' WHERE username = '$username'");
    if($inTrueM){
