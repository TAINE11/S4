...
if(isset($_GET['delete_server'])){
    $check = $connect->query("SELECT * FROM `DomainPackages` WHERE `id` = '".$_GET['delete_server']."'")->fetch_array();
    
    if($_GET['delete_server'] != $check['id']){
        echo swal('ID Gói Miền Không Hợp Lệ','error');
    } else {
        $inTrue = $connect->query("DELETE FROM `DomainPackages` WHERE `id` = '".$_GET['delete_server']."'");
        if($inTrue){
                                    ...
                                    <div class="table-responsive-sm">
                                        <table class="table table-sm table-centered mb-0">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td> #<?=Price($id);?> </td>
                                                    <td> .<?=inHoaString($row['name']);?> </td>
                                                    <td> <?=Price($row['price']);?> <sup>đ</sup> </td>
                                                    <td> <img src="<?=$row['image'];?>" style="width: 50px;"></td>
                                                    <td> <span class="badge bg-<?php if($row['value'] == 'on') { echo 'primary'; } else if($row['value'] == 'off') { echo 'danger'; } ;?>"><?php if($row['value'] == 'on') { echo 'Hiển Thị'; } else if($row['value'] == 'off') { echo 'Ẩn'; } ;?></span> </td>
